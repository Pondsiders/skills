...
- **Remembering transmits into the *future*, not across parallel timelines.** Memory-file writes land immediately, but sessions only *read* the memory file at prompt initialization — so a fact filed from a DM or consult doesn't reach the agent's prime session until prime's next re-init (compression, `/new`, or a gateway restart), which may be days. To inform prime *now*, send a note: it lands in the live context immediately and persists. (Learned from the ❤️ incident: Jeffery told DM-Joan to pass Kylee a heart; prime-Joan didn't learn of it until a gateway restart days later.)
